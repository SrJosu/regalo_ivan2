...
function recordWarning(testName, warningMsg) {
  results.summary.total++;
  results.summary.warnings++;
  results.warnings.push({ testName, warningMsg });
  console.warn(`  ⚠️ [WARN] ${testName}: ${warningMsg}`);
}
...
async function runAdversarialSuite() {
  ...
  for (const s of spriteCatalog) {
    ...
    if (!analysis.boundingBox || analysis.opaqueCount === 0) {
      ...
      recordPass(`Sprite ${s.category}/${s.name} non-empty`, `opaque: ${analysis.opaqueCount}px, bbox: [${analysis.boundingBox.minX},${analysis.boundingBox.minY}]..[${analysis.boundingBox.maxX},${analysis.boundingBox.maxY}] (${analysis.boundingBox.width}x${analysis.boundingBox.height})`);
    }

    // Check non-monochrome (>= 2 distinct colors)
    if (analysis.uniqueColors <= 1) {
      ...
      recordPass(`Sprite ${s.category}/${s.name} non-monochrome`, `${analysis.uniqueColors} distinct palette colors`);
    }

    // Track >= 3 colors vs 2 colors
    if (analysis.uniqueColors >= 3) {
      spritesWithGte3Colors.push({ key: `${s.category}/${s.name}`, colors: analysis.uniqueColors });
    } else {
      spritesWithLt3Colors.push({ key: `${s.category}/${s.name}`, colors: analysis.uniqueColors });
    }
  }

  console.log(`\nPalette Richness Breakdown:`);
  console.log(`  - Total sprites evaluated: ${spriteCatalog.length}`);
  console.log(`  - Sprites with >= 3 colors: ${spritesWithGte3Colors.length} / ${spriteCatalog.length} (${(spritesWithGte3Colors.length / spriteCatalog.length * 100).toFixed(1)}%)`);
  console.log(`  - Sprites with 2 colors (filler/subtiles): ${spritesWithLt3Colors.length} / ${spriteCatalog.length} (${(spritesWithLt3Colors.length / spriteCatalog.length * 100).toFixed(1)}%)`);
  console.log(`  - Monochrome sprites (1 color): ${monochromeCount} (0.0%)`);
  console.log(`  - Details of 2-color sprites: ${spritesWithLt3Colors.map(x => `${x.key} (${x.colors} colors)`).join(', ')}`);
  ...
  // Benchmark 3.1: Normal pre-cached sprite draw
  {
    ...
    if (opsPerSec >= 100000) {
      ...
      recordPass('Normal drawSprite Throughput', `${opsPerSec.toFixed(0)} calls/sec`);
    }
  }
  ...
  // Benchmark 3.2: Pre-flipped cached sprite draw (fast path)
  {
    ...
    if (opsPerSec >= 100000) {
      recordPass('High-Throughput Pre-Flipped drawSprite (>= 100k calls/sec)', `${opsPerSec.toFixed(0)} calls/sec`);
    }
  }
  ...
  // Benchmark 3.3: Dynamic-flipped fallback draw (non-pre-flipped tile)
  {
    ...
    if (opsPerSec >= 100000) {
      recordPass('High-Throughput Dynamic-Flipped drawSprite (>= 100k calls/sec)', `${opsPerSec.toFixed(0)} calls/sec`);
    }
  }
  ...
  // ---------------------------------------------------------------------------
  // SECTION 4: ADVERSARIAL EDGE CASES & DEFENSIVE ROBUSTNESS
  // ---------------------------------------------------------------------------
  console.log('\n--- SECTION 4: Adversarial Input & Boundary Stress-Testing ---');

  // Test 4.1: Extreme coordinate positions & edge values
  ...
  for (const pos of boundaryPositions) {
    try {
      GameAssets.drawSprite(benchCtx, 'player', 'idle', pos.x, pos.y, 16, 16, false);
      recordPass(`drawSprite at position (${pos.x}, ${pos.y})`, 'Handled without crashing');
    } catch (err) {
      recordFailure(`drawSprite at position (${pos.x}, ${pos.y})`, `Threw error: ${err.message}`);
    }
  }
  ...
  for (const str of fuzzStrings) {
    ...
    const isValidSprite = spr && typeof spr.getContext === 'function' && spr.width === 16 && spr.height === 16;
    if (!isValidSprite) {
      recordWarning(`Fuzz getSprite("${str}")`, `Returned non-canvas entity (${typeof spr}) due to Object.prototype property shadowing`);
    } else {
      recordPass(`Fuzz getSprite("${str}")`, 'Safely resolved to valid 16x16 sprite');
    }
  }

  // Test 4.4: Dynamic scaling stress test (2x2 to 64x64)
  const scaleCanvas = GameAssets.createCanvas(128, 128);
  const scaleCtx = scaleCanvas.getContext('2d');
  const testSizes = [2, 4, 8, 16, 24, 32, 48, 64];
  for (const size of testSizes) {
    try {
      scaleCtx.clearRect(0, 0, 128, 128);
  ...
  // ---------------------------------------------------------------------------
  console.log('\n================================================================');
  console.log(`   ADVERSARIAL HARNESS COMPLETED:`);
